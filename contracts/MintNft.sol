...
// File: contracts/artwork/SfpNFT.sol
...
contract SfpNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    function __mint(address _to, string memory _tokenURI) internal returns (uint256 tokenId) {
        tokenId = nextTokenId;
        _mint(_to, tokenId);
        nextTokenId++;
    ...
    function mint(address _to, string memory _tokenURI) public payable returns (uint256 tokenId) {
        ...
        tokenId = __mint(_to, _tokenURI);
        if (msg.value > mintFeeAmount) TransferHelper.safeTransferETH(msg.sender, msg.value - mintFeeAmount);
    }

    function batchMint(address _to, string[] memory _tokenURIs) public payable returns (uint256[] memory tokenIds) {
        uint256 totalMintFee = mintFeeAmount.mul(_tokenURIs.length);
        require(msg.value >= totalMintFee, 'msg value too low');
        TransferHelper.safeTransferETH(mintFeeAddr, totalMintFee);
        tokenIds = new uint256[](_tokenURIs.length);
        for (uint256 i = 0; i < _tokenURIs.length; i++) {
            tokenIds[i] = __mint(_to, _tokenURIs[i]);
        }
        if (msg.value > totalMintFee) TransferHelper.safeTransferETH(msg.sender, msg.value - totalMintFee);
    }
    ...
    function mintBulk(
        ...
        uint256 totalMintFee = mintFeeAmount.mul(_size);
        require(msg.value >= totalMintFee, 'msg value too low');
        TransferHelper.safeTransferETH(mintFeeAddr, totalMintFee);
        tokenIds = new uint256[](_size);
        for (uint256 i = 0; i < _size; i++) {
            tokenIds[i] = __mint(_to, _tokenURI);
        }
        if (msg.value > totalMintFee) TransferHelper.safeTransferETH(msg.sender, msg.value - totalMintFee);
    }
